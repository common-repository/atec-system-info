...
function atec_license_banner():void
{
	...
	<div class="atec-sticky-right">
		<a class="atec-nodeco atec-', ($licenseOk?'green':'blue') ,'" href="', esc_url($link), '">
			...
			($licenseOk?esc_attr__('PRO license activated','atec-system-info'):esc_attr__('Get your PRO „Lifetime-Site-License“','atec-system-info')), '.
		</a>
	</div>';
}
...
{ 
	...
	<div class="atec-header">
		...
		<div class="atec-center">	
			...
			if ($approved)
			{
				echo ' <a style="position:relative; margin-left: 10px;" class="atec-fs-12 atec-nodeco atec-btn-small" href="', esc_url($wordpress.$plugin.'/reviews/#new-post'), '" target="_blank"><span class="', esc_attr(atec_dash_class('admin-comments')), '"></span> ', esc_attr__('Post a review','atec-system-info'), '</a>';
			}		
		echo '
		</div>
	</div>';
